...
template<class MSG_T, size_t reps = 100, class UNIT = std::chrono::microseconds>
void measure_type_introspection(const std::string & prefix = "", bool verbose = false)
{
  int64_t d_min = 999999999;
  int64_t d_max = 0;
  float d_std = 0.0;
...
int main(int argc, char ** argv)
{
  ...
  measure_serialize<test_msgs::msg::UnboundedSequences>(ubs_msgs[0].get(), "unbounded sequences");

  auto bs_msgs = get_messages_bounded_sequences();
  measure_serialize<test_msgs::msg::BoundedSequences>(bs_msgs[0].get(), "bounded sequences");
